...
<div class="bulk-mail-out-form" xmlns="http://www.w3.org/1999/html">
        ...
        <!-- <div class="row">
            ...
            <div class="col-md-6 form-group">
                <select class="form-control" name="last_login" id="last_login">
                    ...
                    <option value="3">Over 30 Days Since Login</option>
                </select>
            </div>
            <div class="col-md-6 form-group">
                <select class="form-control" name="status" id="status">
    ...
    <form
        ...
        action="<?= Url::to(['/email-clients/send-bulk-mail-out']) ?>"
        method="post"
        ...
        <div>
            ...
            <h5>Selected Users <b>(<?= count($selectedUsers) ?>)</b>
                ...
                <br/>
                <?php foreach ($selectedUsers as $email_obj) { ?>
                    <div style='margin-bottom: 2px'>
                        <label><input type='checkbox' checked='checked' value='<?= $email_obj['email'] ?>' name='user_id[]' class='user-checkbox'/> <?= $email_obj['email'] ?> </label>
                    </div>
                    <br/>
                <?php } ?>
            <?php } ?>
            <div class="show_loader hidden"><h3>Loading....</h3></div>
            <div id="show_client_data_table" style="display:none;">
        ...
        <!-- <div class="display-flex-div">
            <p class="width-100-percent">
                <a id="email-users" class="float-right-element btn btn-primary">Email Clients</a>
            </p>
        </div>
        <div class="full-row">
...
<script>
    ...
    $(document).on('change', "#clientEmail", function (e) {
        ...
        data = {filter: $(this).val(), type: 'clientEmail', page: <?= $page ?>, selectAll: 1, isSelectAllClicked: 1};
    ...
    function ajaxEligibleUsers(postData) {
        ...
        $.ajax({
            async: false,
            url: '<?= Url::to(['email-clients/eligible-email-users']) ?>',
            type: 'post',
            data: postData,
            success: function (response) {
                let obj = JSON.parse(response);
                $('.show_loader').addClass('hidden');
                $('#show_client_data_table').show();
                $('#show_client_data_table').html(obj.users);
            },
            error: function () {
                console.log("error");
            }
