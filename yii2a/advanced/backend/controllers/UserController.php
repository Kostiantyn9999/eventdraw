...
class UserController extends Controller
{
    ...
    public function actionExportfiltered()
    {

       Yii::$app->response->format = \yii\web\Response::FORMAT_RAW;
    Yii::$app->response->headers->add('Content-Type', 'text/csv; charset=UTF-8');
    Yii::$app->response->headers->add('Content-Disposition', 'attachment; filename="filtered_users.csv"');
    ...
    fputcsv( $handle, [ 'ID', 'Client','First Name','Surname','Email','Login',
    ...
    foreach ($dataProvider->models as $model) {
        ...
        {
            $last_login_good =  \Yii::$app->formatter->asDatetime($model->last_login, "php:d-M-Y H:i:s");
        }
        else
        {
            $last_login_good = '';
        ...
        if ($model->expiry_date != 0)
        {
            $expiry_date_good =  \Yii::$app->formatter->asDatetime($model->expiry_date, "php:d-M-Y H:i:s");
        }
        else
        {
            $expiry_date_good = '';
        ...
        {
            $created_at_good =  \Yii::$app->formatter->asDatetime($model->created_at, "php:d-M-Y");
        }
        else
        {
            $created_at_good = '';
        ...
        {
            $updated_at_good =  \Yii::$app->formatter->asDatetime($model->updated_at, "php:d-M-Y H:i:s");
        }
        else
        {
            $updated_at_good = '';
    ...
    Yii::$app->response->content = $csvOutput;
