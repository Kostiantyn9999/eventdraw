			...
			/*if (req.body.html)
			{
				...
				try
				{
					...
					browser = await puppeteer.launch({
						args: ['--headless','--disable-gpu', '--no-sandbox','--ignore-certificate-errors', '--disable-setuid-sandbox', '--disable-dev-shm-usage']
					});
			...
			{	
				...
				if (req.body.format && xml && req.body.w * req.body.h <= MAX_AREA)
				{
					...
					try
					{
						...
						browser = await puppeteer.launch({
							args: ['--ignore-certificate-errors','--headless','--disable-gpu', '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage']
						});
