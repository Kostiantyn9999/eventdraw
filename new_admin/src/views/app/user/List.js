...
import { getUserList, deleteUser, resetUser } from "reduxs/actions";
...
const UserList = (props) => {
  ...
  const { userList, delLoading, loading, success } = useSelector((state) => state.user);
  ...
  const onChange = (search, sort, page, perPage, col) => {
    const filteredList = userList.sort((a, b) => sortFunc(a, b, sort, col)).filter((item) => {
  ...
  const handleEditClick = (e, id) => {
    e.preventDefault();
    navigate(`${process.env.PUBLIC_URL}/user/edit/${id}`);
  };
  ...
  useEffect(() => {
    if (!userList) return;
    const to = metaData.to < userList.length ? metaData.to : userList.length;
    setMetaData({ ...metaData, total: userList.length, totalPages: Math.ceil(userList.length / perPage), to: to });
    setData([...userList.sort((a, b) => sortFunc(a, b, sortOrder, activeCol)).slice(0, perPage)]);
  }, [userList]);
  ...
  return (
    <>
      ...
      <StyledCard>
        ...
        >
          <Toolbar
            ...
            allowButtonA={true}
            buttonAIcon={<AddIcon />}
            handleButtonA={() => navigate(`${process.env.PUBLIC_URL}/user/add`)}
            search={search}
      ...
      <AlertDialog
        ...
        title="Delete"
        info="Are you sure to delete selected user?"
