  ...
  return (
    <Mui.Box display="flex" flexDirection="column" mb={5} className={`${props.classes?.root || ""}`}>
      ...
      <Mui.Breadcrumbs separator="›" sx={{ fontSize: "0.875rem" }}>
        <Item to={`${process.env.PUBLIC_URL}/dashboard`}>Dashboard</Item>
          ...
          return (
            <span key={index}>
              {index === props.paths.length - 1 ? (
                ...
                <Item key={index} to={index === props.paths.length - 1 ? null : `${process.env.PUBLIC_URL}${item.page}` ? `${process.env.PUBLIC_URL}${item.page}` : `${process.env.PUBLIC_URL}`}>
                  {item?.title}
                </Item>
              )}
            </span>
