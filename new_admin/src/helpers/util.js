...
export const monetarySuffix = (val) => {
  if (val >= 1000) {
    return parseFloat(val / 1000).toFixed(5) + "K";
  } else if (val % 1 !== 0) {
...
export const GetSDK = (elementId, applicationKey) => {
    ...
    const checkIframe = async () => {
      ...
      if (iframe) {
        ...
        if (iWindow != null && iWindow.MP_SDK) {
          ...
          const sdk = await iWindow.MP_SDK.connect(iWindow, applicationKey);
          resolve(sdk);
        }
      }
    };
...
export const mid = (min, max) => {
  const result = (max - min) / 2 + min;
  return parseFloat(result.toFixed(5));
};

export const getDimension = (width, height, center) => {
  ...
  return {
    xMin: parseFloat(xMin.toFixed(5)),
    xMax: parseFloat(xMax.toFixed(5)),
    yMin: parseFloat(yMin.toFixed(5)),
    yMax: parseFloat(yMax.toFixed(5)),
  };
};

export const loadAsyncFile = (file) => {
  return new Promise((resolve, reject) => {
...
export const roundToNearestTarget = (number) => {
