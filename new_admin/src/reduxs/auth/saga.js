    ...
    if (response.data.success) {
      ...
      payload.navigate(
        response?.data?.data?.token
          ...
          : `${process.env.PUBLIC_URL}/auth/initial-change-password/${response?.data?.data?.userId}/${response?.data?.data?.verificationCode}`
      );
    } else {
      toaster("", response.data.message);
      yield put(verifyUserError(response.data.message));
      payload.navigate(`${process.env.PUBLIC_URL}/auth/login`);
    }
  } catch (error) {
    const errMessage = parseMessage(handleResponseErrorMessage(error));
    payload.navigate(`${process.env.PUBLIC_URL}/auth/login`);
...
function* initialChangePassword({ payload }) {
  try {
    ...
    if (response.data.success) {
      ...
      localStorage.setItem("currentUser", JSON.stringify(response.data.data));
      localStorage.setItem("token", response.data.data.token);
      payload.navigate(`${process.env.PUBLIC_URL}/dashboard`);
    } else {
...
function* login({ payload }) {
  try {
    ...
    if (response.data.access_token) {
      ...
      toaster("", response.data.message ? response.data.message : "Login Fail!");
